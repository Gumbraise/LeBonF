        ...
        if(isset($_POST['text'])) {
            if(isset($_POST['price'])) {
                if(isset($_POST['select'])) {
                    if(isset($_FILES['file']['name'])) {
                        if($_POST['price'] > 0) {
                            ...
                            if($imageFileType == "jpg" || $imageFileType == "png" || $imageFileType == "jpeg" || $imageFileType == "gif") {
                                ...
                                {
                                    $requser = $bdd->prepare("INSERT INTO vente(user_id, text, picture, price, date, categorie) VALUES(?, ?, ?, ?, UNIX_TIMESTAMP(), ?)");
                                    $requser->execute(array($_SESSION['id'], $_POST['text'], $newfileforsql, $_POST['price'], $_POST['select']));
                                    echo "ok";
                                }
                            } else {
                    ...
                    echo 'Vous n\'avez pas selectionné de categorie';
                }
            } else {
